...
impl AppManagerInner {
    ...
    pub fn get_app_start(&self) -> usize {
        self.app_start[self.current_app]
    }
    
    pub fn get_app_end(&self) -> usize {
...
pub fn in_valid_space(addr : usize, len : usize, ptr : usize) -> bool {
    let size = APP_MANAGER.inner.borrow().get_app_end() - APP_MANAGER.inner.borrow().get_app_start();
    let sp = USER_STACK.get_sp();
    // let stack_size = sp - ptr;
    (addr >= APP_BASE_ADDRESS && (addr + len) <= (APP_BASE_ADDRESS + APP_SIZE_LIMIT) ) || ((addr + len <= sp) && (addr >= ptr) )
}
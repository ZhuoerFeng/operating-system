...
pub fn sys_mmap(start: usize, len: usize, port: usize) -> isize {
    ...
    let a = insert_va(v_start, v_end, flag);
    if a == -1 {
        println!("[Kernel] MMAP failed");
        exit_current_and_run_next(114514);
        panic!("Unreachable in mmap!");
    } else if a == 2 {
        return -1;
    }
    size as isize
...
pub fn sys_fork() -> isize {
    ...
    if let Some(new_task) = current_task.fork() {
        ...
        println!("[Kernel] Fork new task fail!");
        exit_current_and_run_next(114514);
        panic!("Unreachable in sys_fork\n");
    }
}
...
pub fn sys_spawn(path: *const u8) -> isize {
    ...
    if let Some(app_inode) = open_file(path.as_str(), OpenFlags::RDONLY) {
        ...
        if let Some(new_task) = current_task.fork() {
            ...
            println!("[Kernel] Fork new task fail!");
            exit_current_and_run_next(114514);
            panic!("Unreachable in sys_spawn\n");
        }
    } else {
        -1
    }
